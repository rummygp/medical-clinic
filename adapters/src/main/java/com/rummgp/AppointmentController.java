    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Internal server error",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = ErrorMessageDto.class))})})
    @GetMapping
    public PageDto<AppointmentDto> find(@ModelAttribute AppointmentFindRequestDto appointmentFindRequestDto) {
        AppointmentFindCommand command = appointmentMapper.toFindCommand(appointmentFindRequestDto);
        return pageMapper.toDto(appointmentService.find(command), appointmentMapper::toDto);
    }
